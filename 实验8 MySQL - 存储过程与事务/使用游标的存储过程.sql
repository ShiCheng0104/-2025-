...
begin
	...
	open cur1;
	open cur2;
	while start_date <= end_date do fetch cur1 into nur1;
	if done then close cur1;
	open cur1;
	set done = false;
	...
	if wk = 0
	and head is not null then
	set doc = head;
	set head = null;
	else fetch cur2 into tp, doc;
	if done then close cur2;
	open cur2;
	set done = false;
	fetch cur2 into tp, doc;
	end if;
	if wk > 4 and tp = 1 then set head = doc;
	fetch cur2 into tp, doc;
	if done then close cur2;
	open cur2;
	set done = false;
	...
	values (start_date, doc, nur1, nur2);
